...
<div class="card">
    <div class="card-header">
        <h2 class="text-dark text-2xl font-medium">
            {{ $transaction->order->badminton_field->name }}
        </h2>
        ...
        <div class="pl-2">
            ...
            <p class="text-gray-500 text-sm font-medium"><i class="fas fa-xs fa-clock"></i> {{ $timeStart }} - {{
                $timeEnd }} WIB ({{ $transaction->order->hours }}
                jam)
            </p>
        </div>
        <div>
            ...
            <div class="pl-2">
                <div class="flex justify-between items-center">
                    ...
                    <p
                        class="py-1 px-3 text-black text-xs font-medium {{ $transaction->transaction_type_id == 2 ? 'bg-green-500' : 'bg-blue-500' }} rounded-md">
                ...
                <div class="flex justify-between items-center">
                    ...
                    <span>
                        ...
                        <p class="py-1 px-3 text-black text-xs font-medium bg-green-500 rounded-md">
                            Lunas
                        </p>
                        @else
                        <p
                            class="py-1 px-3 text-black text-xs font-medium {{ $transaction->order->status_transaction->color }} rounded-md">
                ...
                @if ($transaction->order->status_transaction_id < 3) <form
                    class="flex flex-col items-center align-middle"
                    action="{{ route('app.transaction.pay',['transaction'=>$transaction->id]) }}" method="post"
